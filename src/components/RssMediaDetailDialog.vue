    ...
    <q-card class="detail-card">
      ...
      <q-card-section v-if="item" class="detail-body">
        ...
        <div class="detail-content">
          ...
          <div class="detail-meta"><q-badge :color="stateMeta(item.state).color" :label="stateMeta(item.state).label" /><span v-if="doneOf(item)" class="muted">{{ doneOf(item) }}</span><q-linear-progress v-if="type === 'TV' && item.total && item.total > 0" :value="progressOf(item) / 100" color="primary" size="6px" class="detail-progress" /></div>
